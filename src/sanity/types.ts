...
// Query: count(*[_type == "journal"    && ($hasTags == false || count(tags[@->slug.current in $tags]) > 0)  ])
export type JOURNAL_COUNT_QUERY_RESULT = number;

// Source: src/sanity/lib/queries.ts
// Variable: JOURNAL_TAGS_QUERY
// Query: array::unique(*[_type == "journal" && defined(tags)].tags[]->{ _id, name, "slug": slug.current })
export type JOURNAL_TAGS_QUERY_RESULT = Array<{
  _id: string;
  name: string;
  slug: string;
}>;
