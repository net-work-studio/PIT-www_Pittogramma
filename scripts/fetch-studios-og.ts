/**
 * Bulk script: Fetch OG data for all studio documents
 *
 * Run with:
 *   bun run scripts/fetch-studios-og.ts
 *
 * What it does:
 * 1. Fetches all studio documents with a "website" social link
 * 2. Fetches OG metadata (title, description, siteName, imageUrl) from each website
 * 3. Downloads and uploads OG images to Sanity
 * 4. Patches each studio document with the fetched data
...
async function main() {
  console.log("=== Fetch OG Data for Studios ===\n");

  const docs: StudioDoc[] = await client.fetch(
  ...
  // Filter to studios that have a website URL
  const withUrl = docs.filter(
    (d): d is StudioDoc & { websiteUrl: string } => !!d.websiteUrl
  );

  console.log(
    `Found ${docs.length} studios total, ${withUrl.length} with website URLs\n`
  );
